...
const settingColumns: DynamicTableColumn<PlatformSetting>[] = [
  {
    ...
    renderCell: (row) => (
      <div>
        ...
        <p className="text-xs text-muted">{row.settingKey}</p>
      </div>
    ),
  },
  ...
  { key: "valueType", label: "Type", minWidth: 100 },
  {
    ...
    label: "Editable",
    format: "status",
    statusTone: (value) => (value ? "success" : "neutral"),
    renderCell: (row) => (row.isEditable ? "YES" : "NO"),
  },
...
const categoryColumns: DynamicTableColumn<ServiceCategory>[] = [
  {
    ...
    renderCell: (row) => (
      <div>
        ...
        <p className="text-xs text-muted">{row.categoryCode}</p>
      </div>
    ),
  },
  {
    ...
    renderCell: (row) => (
      <div>
        <p className="font-semibold text-foreground">
          {row.bookingTemplate?.defaultPricingMode ?? "Not configured"}
        </p>
        <p className="text-xs text-muted">
          {row.bookingTemplate?.multiServiceEnabled
            ? "Multi-service"
            : "Single service"}{" "}
          · {row.bookingTemplate?.quoteMode ?? "INSTANT"}
        </p>
      </div>
    ),
  },
  {
    ...
    label: "Status",
    format: "status",
    statusTone: (value) => (value ? "success" : "danger"),
...
const zoneColumns: DynamicTableColumn<ServiceZone>[] = [
  {
    ...
    renderCell: (row) => (
      <div>
        <p className="font-semibold text-foreground">{row.zoneName}</p>
        <p className="text-xs text-muted">{row.zoneId}</p>
      </div>
    ),
  },
  ...
  },
  {
    key: "pincodeList",
    label: "Pincodes",
    renderCell: (row) => row.pincodeList.length,
  },
  { key: "updatedAt", label: "Updated", format: "date", minWidth: 180 },
];
...
export function SettingsPage() {
  ...
  const mutation = useMutation<unknown, Error, SettingsActionFormValues>({
  ...
  return (
    <PageContainer>
      <PageContextHeader
        title="Settings"
        description="Manage platform settings, categories, and service zones."
        placement="topbar"
      />
      <div className="list-workspace">
        <ListFilterBar
          actionNode={
            <div className="flex flex-wrap gap-2">
              {(
                ["settings", "categories", "zones"] as SettingsRecordType[]
              ).map((item) => (
                <Button
                  key={item}
                  size="sm"
              ...
              {type === "zones" ? (
                <Button
                  size="sm"
                  onClick={() =>
                    setSelectedAction({ type: "zones", action: "CREATE" })
                  }
                >
        ...
        <section className="list-results-panel">
          {result.isError ? (
            ...
            <TableSkeleton
              columns={columns as DynamicTableColumn<unknown>[]}
            ...
            <DynamicTable
              columns={columns as DynamicTableColumn<Row>[]}
              data={rows}
              getRowId={(row) => getRowId(type, row)}
              title={type}
              onRowClick={(row) =>
                type === "categories"
                  ? setSelectedAction({
